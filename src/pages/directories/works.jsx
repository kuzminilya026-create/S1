...
export default function WorksPage() {
  ...
  const handleAddMaterial = async (values) => {
    try {
      const response = await fetch(`http://localhost:3002/api/works/${selectedWork.id}/materials`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleRemoveMaterial = async (materialId) => {
    try {
      const response = await fetch(`http://localhost:3002/api/works/${selectedWork.id}/materials/${materialId}`, {
        method: 'DELETE'
      });
